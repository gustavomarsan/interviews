...
*** Note: Doing only 2 loops, 1 from left to right and 1 from right to left
"""
...
def minOperations(boxes: str) -> list[int]:
    ...
    prev = 0
    balls = 0
    for i in range(len(boxes)) :  # left to right
        answer[i] = prev
        if boxes[i] == "1" :
            balls += 1
        prev += balls
    prev = 0
    balls = 0
    for i in range(len(boxes) -1, -1, -1) : # right to left
        answer[i] += prev
        if boxes[i] == "1" :
            balls += 1
        prev += balls
